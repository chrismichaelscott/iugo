...
$iugo.$internals.MVVC.prototype.initializers.push(function(view) {
	...
	view.innerHTML = view.innerHTML.replace(tagRegex, function(tag) {
		...
		var tag = tag.replace(attributeRegex, function(attribute, attributeName, attributeValue) {
			...
			attributeValue.replace(namespaceRegex, function(m, namespace) {
				...
				if (!$iugo.store.bind_by_class.namespaces[namespace]) {
					$iugo.store.bind_by_class.namespaces[namespace] = [];
				}
				// Record that changes to the namespace will need to recompile this tag
				if ($iugo.store.bind_by_class.namespaces[namespace].indexOf(tagId) == -1) {
					$iugo.store.bind_by_class.namespaces[namespace].push(tagId);
				}
			});
...
$iugo.$internals.MVVC.prototype.defaultViewcontrollers.push(function(property, value, view, path) {
	...
	if ($iugo.store.bind_by_class.namespaces[property]) {
		var idList = $iugo.store.bind_by_class.namespaces[property];
