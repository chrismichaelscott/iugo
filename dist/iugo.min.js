...
$iugo.defaultViewcontrollers.push(function(a,c){function b(c){for(var b=document.querySelector('[data-iugo_id="'+c+'"]'),d=a.store.tags[c].bindAttributes,f=0;f<d.length;f++){var e=a.store.tags[c].attributeTemplates[d[f]].replace(k,function(b){return a.store.tags[c].replacements[b]?a.store.tags[c].replacements[b]:""});b.setAttribute(d[f],e)}}function f(c,d){if(d.hasAttribute("data-iugo_id")){for(var f=d.getAttribute("data-iugo_id"),e=function(b,d,e){var l=null,l=d?a.model[d.substr(0,d.length-1)]:c;
d=e.split(".");for(e=0;e<d.length;e++)if(""!==d[e])if(l[d[e]])l=l[d[e]];else throw"bad bind address";a.store.tags[f].replacements[b]=l},h=a.store.tags[f].bindAttributes,r=0;r<h.length;r++)a.store.tags[f].attributeTemplates[h[r]].replace(k,e);b(f)}}function d(c){var b=c.getAttribute("data-iugo_id"),d=a.store.idCounter++,b=a.store.tags[b];a.store.tags[d]={bindAttributes:b.bindAttributes,attributeTemplates:b.attributeTemplates,replacements:{}};c.setAttribute("data-iugo_id",d)}function e(c,b,k){var g=
b.getAttribute("data-bind");if(g){var h=g.indexOf(":");0<h&&(c=a.model[g.substr(0,h)],g=g.substr(h+1));try{g.split(".").forEach(function(a){if(""!==a)if(c[a])c=c[a];else throw"bad bind address";})}catch(r){return}}if((g=b.getAttribute("data-if"))&&(g=a.controller[g])&&g instanceof Function)if(g.call(b,c,a))g=b.getAttribute("data-iugo_olddisplay"),b.style.display=g?g:"";else{b.hasAttribute("data-iugo_olddisplay")||b.setAttribute("data-iugo_olddisplay",b.style.display);b.style.display="none";return}f(c,
b);if(c instanceof Array){g=b.children.length;for(h=g-1;0<=h;h--)b.children[h].classList.contains("iugo_cloned")&&b.removeChild(b.children[h]);g=b.children.length;for(h=0;h<g;h++){var m;if(b.children[h].hasAttribute("data-each")){m=b.children[h];0===c.length?(m.setAttribute("data-iugo_display",m.style.display),m.style.display="none"):m.style.display=m.getAttribute("data-iugo_display");for(var s=0;s<c.length;s++){var n;if(1<=s){n=m.cloneNode(!0);n.classList.add("iugo_cloned");n.hasAttribute("data-iugo_id")&&
d(n);for(var u=n.querySelectorAll("[data-iugo_id]"),t=0;t<u.length;t++)d(u[t]);b.appendChild(n)}else n=m;e(c[s],n,k)}}}}else if(0<b.children.length)for(h=0;h<b.children.length;h++)e(c,b.children[h],k);else c instanceof Object||("INPUT"==b.tagName?b.value=c:b.innerHTML=c)}var k=/\{\{([^:}]+:)?([^}]+)\}\}/g;e(a.model,a.scope,"")});
$iugo.initializers.push(function(a){"click contextmenu hoverin hoverout focus blur change".split(" ").forEach(function(c){a.scope[getRealEvent(c)]=passEventToController(c,a)})});function getRealEvent(a){"hoverin"==a&&(a="mouseover");"hoverout"==a&&(a="mouseout");return"on"+a}function nodeToPath(a){for(var c=[];a;a=a.parentNode)c.push(a);return c}
function getPathFromSplit(a,c){for(var b=null,f=nodeToPath(a);c;c=c.parentNode){for(var d=0;d<f.length;d++)if(c.isEqualNode(f[d])){for(var b=[],e=0;e<d;e++)b[e]=f[e];break}if(b)break}return b}
...
window.Iugo.http={get:function(a,c,b,f){return this.xhr("get",a,void 0,{},b,f)},xhr:function(a,c,b,f,d,e,k){if(window.Promise)return new Promise(function(p,l){var q=new XMLHttpRequest;q.onload=function(){200<=this.status&&300>this.status?f instanceof Function?p(f(this.responseText)):"application/json"==this.getResponseHeader("content-type")?p(JSON.parse(this.responseText)):p(this.responseText):l(this)};q.onerror=q.onabort=function(){l(this)};q.open(a,c,!0,e,k);for(var g in d)d.hasOwnProperty(g)&&
q.setRequestHeader(g,d[g]);q.send(b)});console.log("To use the HTTP extension, load a Promise polyfill")}};
